...
export const ImageUpload = ({ listOfIimages, type }) => {
    ...
    return (
        <div className="imageUpload_wrap" style={{ background: 'none' }}>
            <ImageUploading multiple value={images} onChange={onChange} maxNumber={maxNumber} dataURLKey="data_url" acceptType={["jpg", "png", "gif"]}>
                {({ imageList, onImageUpload, onImageRemoveAll, onImageUpdate, onImageRemove, isDragging, dragProps }) => (
                    ...
                    <div className="upload__image-wrapper" style={{ background: 'none' }}>
                        {/* If type is single after you upload the image it should hide this button */}
                        {imageList.length === 0 ? <button className='imageUpload_btn1' style={isDragging ? { color: "red" } : null} onClick={onImageUpload} {...dragProps}
                        >
                            <div>
                                <i class="ph-bold ph-upload-simple" style={{ fontSize: '20px' }} />
                            </div>
                            <div style={{ fontFamily: 'Nunito Sans' }}>Profile<br /> Image</div>
                        </button> : ''}
                        <div style={{ height: '0px' }}>&nbsp;</div>
                        {imageList.length < 2 ? '' : <button onClick={onImageRemoveAll}>Remove all images</button>}
                        {imageList.map((image, index) => (
                            <div key={index} className="image-item" style={{ alignItems: 'center', justifyContent: 'flex-end' }}>
