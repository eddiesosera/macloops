...
export const ImageUpload = ({ listOfIimages, type, maxNumber, imageLabel }) => {
            ...
            <ImageUploading multiple value={images} onChange={onChange} maxNumber={maxNumber} dataURLKey="data_url" acceptType={["jpg", "png", "gif"]} >
                {({ imageList, onImageUpload, onImageRemoveAll, onImageUpdate, onImageRemove, isDragging, dragProps }) => (
                    ...
                    <div className="upload__image-wrapper" style={{ background: 'none', width: '100%' }}>
                        {/* If type is single after you upload the image it should hide this button */}
                        {
                            imageList.length !== maxNumber ?
                                <button className='imageUpload_btn1'
                                    ...
                                    <div style={{ fontFamily: 'Nunito Sans' }}>{imageLabel}</div>
                                </button>
                                :
                                ''
                        }
                        ...
                        <div style={{ display: 'flex', gap: '5px', maxWidth: '265px', flexWrap: 'wrap', justifyContent: 'flex-wrap' }}>
                            {imageList.map((image, index) => (
                                <div key={index} className="image-item" style={{ alignItems: 'center', justifyContent: 'flex-start', margin: maxNumber === 1 ? '0 auto' : '' }}>
